...
def sizeChecker(lineNumber, content, optimalSize):
    ...
    if sizeTracker >= 8748071:
        print('returned when larger than 874')
        print(sizeTracker)
        return sizeTracker
    else:
        print('returned when not larger than 874')
        print(optimalSize)
        return optimalSize

with open ('DIRLog.txt') as file:

    optimalSize = 30000000
    nextSize = 0

    content = file.readlines()
    for line in range(len(content)):
        if content[line] == '$ ls\n':
            nextSize = sizeChecker(line,content, optimalSize)
        if nextSize < optimalSize and nextSize != 0:
            print('yep')
            optimalSize = nextSize
    print(optimalSize)